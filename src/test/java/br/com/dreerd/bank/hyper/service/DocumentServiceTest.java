...
@SpringBootTest
public class DocumentServiceTest extends GeneratorInformation implements BaseServiceTest {
    @InjectMocks
    ...
    private List<Document> entityList = new ArrayList<>();

    private DocumentServiceTest() {
    }

    @BeforeEach
    void setUp() {
        entity = createDocument(Document.class);
    ...
    public void mustReturnSuccessWhenList() {
        ...
        List<DocumentDTO> resultList = service.list();
    ...
    public void mustReturnSuccessWhenFindById() {
        Optional<Document> entityOptional = Optional.of(entity);
        when(repository.findById(any())).thenReturn(entityOptional);

        DocumentDTO result = service.findById(entity.getId());
    ...
    public void mustReturnSuccessWhenSave() {
        ...
        DocumentDTO result = service.save(entity.toDTO());
        assertEquals(entity.getId(), result.getId());
        assertNotNull(result);
    ...
    public void mustReturnSuccessWhenEdit() throws JsonProcessingException {
        ...
        DocumentDTO result = service.edit(entity.toDTO());
        assertEquals(entity.getId(), result.getId());
        assertNotNull(result);
        verify(repository, times(1)).findById(any());
    }
